...
class FreeplayCard extends FlxSprite
{
	...
	public function new(x:Float, y:Float, song:String, namer:String, colord:FlxColor, portraits:String, ?requirement:RequireType, ?songs:Array<String>,
			_price:Int = 0, ?forceUnlock = false)
	{
		trueX = x;
		trueY = y;
		iconName = namer;
		...
		if (requirement != SPECIAL)
			lock.frames = Paths.getSparrowAtlas('freeplay/lock', 'impostor');
		else
			lock.frames = Paths.getSparrowAtlas('freeplay/lockGold', 'impostor');
